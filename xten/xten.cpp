...
void xten::createorder(const uint64_t id, const account_name creator) {
  require_auth(_self);

  // eosio::action(
  //    eosio::permission_level{ _self, N(active) },
  //    N(eosio.token), N(transfer),
  //    std::make_tuple(_self, creator, eosio::asset(2500000, S(4, ITS)),
  //    std::string(""))
  // ).send();
...
void xten::createoffer(const uint64_t id, const uint64_t order_id,
  ...
  //    N(eosio.token), N(transfer),
  //    std::make_tuple(contractor, _self, eosio::asset(1000000, S(4, ITS)),
  //    std::string(""))
  // ).send();
...
void xten::createdoc(const uint64_t id, const uint64_t offer_id) {
